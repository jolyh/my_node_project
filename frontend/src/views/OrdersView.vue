    ...
    <main>
        ...
        <ul v-if="orders.length" class="list">
          <li v-for="order in orders" :key="order.id" class="list-item">
            <div class="record-summary">
              ...
              <span>#{{ order.id }} · User #{{ order.userId }} · {{ order.quantity }} items · {{ formatStatus(order.status) }}</span>
            </div>
            <span class="form-actions">
              <button class="btn" type="button" @click="getOrder(order.id)">View</button>
              <button class="btn" type="button" @click="editOrder(order)">Edit</button>
              <button class="btn btn-danger" type="button" @click="deleteOrder(order.id)">Delete</button>
            </span>
          </li>
        </ul>
      ...
      <section v-if="selectedOrder" class="card card-wide">
        ...
        <dl class="profile-details">
          ...
          <div><dt>Status</dt><dd>{{ formatStatus(selectedOrder.status) }}</dd></div>
...
const emptyOrderForm = () => ({ userId: '', productId: '', quantity: 1, totalPrice: 0, status: 0, deliveryDate: '' })
...
const formatStatus = (status) => orderStatuses.find((orderStatus) => orderStatus.value === Number(status))?.label ?? 'Unknown'
