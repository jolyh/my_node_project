...
const createDbInstance = ({
  ...
  nukeOnInit = process.env.NUKE_DATABASE_ON_INIT === 'true' || config.db.NUKE_DATABASE_ON_INIT === true
} = {}) => {
  ...
  const dbInstance = {
    async init() {
      ...
      if (nukeOnInit && process.env.NODE_ENV === 'test') await database.nukeAndRecreate(databaseName);
      else await database.createIfNotExists(databaseName);

      poolInstance = mysql.createPool(dbConfig);
      await poolInstance.execute('SELECT 1');
      await database.setup(poolInstance);
    },
