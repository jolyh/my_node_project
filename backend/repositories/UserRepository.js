...
class UserRepository {
    ...
    tableName = process.env.DB_USER_TABLE || "users";
    ...
    constructor(db) {
        if (!db) throw new AppError(
            ...
            "UserRepository requires a valid database instance"
        );
        this.db = db;
    ...
    async create(user, options = {}) {
        const client = options.connection || this.db; // Use provided client or default to this.db
        const toInsert = User.forCreation(user);
        const { query, values } = this.queryBuilder
            .insert(toInsert)
            .toSQL();
    ...
    */
    async update(id, user, options = {}) {
        const client = options.connection || this.db;
        const toUpdate = User.forUpdate(user);
        const { query, values } = this.queryBuilder
            .update(toUpdate)
            .where('id', '=', id)
            .toSQL();
